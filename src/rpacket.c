...
rpacket_t rpacket_create(allocator_t buf_allo,allocator_t _alloc,buffer_t b,uint32_t pos,uint32_t pk_len,uint8_t is_raw)
{
	rpacket_t r = (rpacket_t)ALLOC(_alloc,sizeof(*r));	
	r->allocator = _alloc;
	r->buffer_allo = buf_allo;
	r->binbuf = 0;
	r->binbufpos = 0;
...
rpacket_t rpacket_create_by_wpacket(allocator_t buf_allo,allocator_t _alloc,struct wpacket *w)
{
	rpacket_t r = (rpacket_t)ALLOC(_alloc,sizeof(*r));	
	r->buffer_allo = buf_allo;
	r->allocator = _alloc;
...
static const void* rpacket_raw_read_binary(rpacket_t r,uint32_t *len)
{
	...
	else
	{
		...
		{
			r->binbufpos = 0;
			r->binbuf = buffer_create_and_acquire(r->buffer_allo,0,r->len);
		}
